...
export default function DeskLocation() {
  ...
  return (
    <Screen>
      <Title>{t("features.deskLocation.title")}</Title>
      <Subtitle>{t("features.deskLocation.subtitle")}</Subtitle>

      <Card className="mt-4">
        {pendingRequest ? (
          <>
            <Row className="items-center justify-between">
              <Text className="font-sansmed text-[14px] text-ink">{t("features.deskLocation.updateRequested")}</Text>
              <Badge label={t("features.deskLocation.pendingApproval")} tone="warn" />
            </Row>
            <Text className="mt-1 font-sans text-[13px] text-faint">
              {t("features.deskLocation.submittedWaiting", { time: fmtDateTime(pendingRequest.created_at) })}
            </Text>
          </>
        ) : (
          <>
            <Text className="font-sansmed text-[14px] text-ink">{t("features.deskLocation.updateYourLocation")}</Text>
            <Text className="mt-1 font-sans text-[13px] text-faint">
              {t("features.deskLocation.usesCurrentLocation")}
            </Text>
            {error && <ErrorText>{error}</ErrorText>}
            <Button
              label={t("features.deskLocation.requestUpdate")} variant="dark" className="mt-3"
      ...
      <Text className="mb-2 mt-6 font-sansbold text-[14px] text-ink">{t("features.deskLocation.history")}</Text>
      {(data.data?.history.length ?? 0) === 0 && (
        <Card><Text className="font-sans text-[13px] text-faint">{t("features.deskLocation.noLocationSet")}</Text></Card>
      )}
      ...
      <Modal visible={confirmOpen} animationType="fade" transparent onRequestClose={() => setConfirmOpen(false)}>
        <View className="flex-1 items-center justify-center bg-ink/40 px-6">
          <View className="w-full rounded-2xl bg-paper p-5">
            <Text className="font-display text-lg text-ink">{t("features.deskLocation.confirmTitle")}</Text>
            <Text className="mt-1 font-sans text-[13px] text-faint">
              {t("features.deskLocation.confirmBody")}
            </Text>
            {error && <ErrorText>{error}</ErrorText>}
            <Row className="mt-4 gap-2">
              <Button label={t("features.deskLocation.cancel")} variant="outline" className="flex-1" onPress={() => setConfirmOpen(false)} />
              <Button label={t("features.deskLocation.sendRequest")} variant="dark" className="flex-1"
