      ...
      <Row className="mt-4 gap-2">
        <Pressable onPress={() => setTab("knowledge")} className="flex-1">
          <View className={`items-center rounded-xl border py-2.5 ${tab === "knowledge" ? "border-espresso bg-espresso" : "border-line bg-paper"}`}>
            <Text className={`font-sansbold text-[13px] ${tab === "knowledge" ? "text-cream" : "text-ink"}`}>Knowledge</Text>
          </View>
        </Pressable>
        <Pressable onPress={() => setTab("sharing")} className="flex-1">
          <View className={`items-center rounded-xl border py-2.5 ${tab === "sharing" ? "border-espresso bg-espresso" : "border-line bg-paper"}`}>
            <Text className={`font-sansbold text-[13px] ${tab === "sharing" ? "text-cream" : "text-ink"}`}>Sharing</Text>
          </View>
        </Pressable>
      </Row>

      <TextInput
        value={search} onChangeText={setSearch} placeholder="Search posts…" placeholderTextColor="#8a8580"
      ...
      <QueryBoundary query={posts}>
        {(rows) => {
          ...
          return (
            <View className="mt-3">
              {sorted.length === 0 && (
                <EmptyText>{tab === "knowledge" ? "No posts found." : "No sharing posts yet — start one."}</EmptyText>
              )}
              {sorted.map((post) => (
                  ...
                  <Pressable>
                    <Card className="mb-2">
                      <Row className="justify-between">
                        ...
                        {post.must_acknowledge && <Badge label="must read" tone="warn" />}
                      </Row>
