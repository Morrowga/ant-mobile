...
export default function NotificationPreferencesScreen() {
  ...
  return (
    <Screen>
      <Subtitle>{t("features.notificationPreferences.subtitle")}</Subtitle>
                ...
                return (
                  <Card key={category} className={`mb-2 flex-row items-center justify-between py-3 ${isLocked ? "opacity-50" : ""}`}>
                    <View className="flex-1 pr-3">
                      ...
                      {isLocked && <Text className="font-sans text-xs text-faint">{t("features.notificationPreferences.alwaysOn")}</Text>}
                    </View>
                    <Switch
                      value={isOn}
